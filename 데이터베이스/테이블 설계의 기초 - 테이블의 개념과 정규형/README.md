...
![사물과 집합의 계층성을 무시한 실패](https://github.com/0jun0815/YJStudy/blob/master/데이터베이스/테이블%20설계의%20기초%20-%20테이블의%20개념과%20정규형/images/사물과%20집합의%20계층성을%20무시한%20실패.jpg)
...
![업계 테이블의 엔터티](https://github.com/0jun0815/YJStudy/blob/master/데이터베이스/테이블%20설계의%20기초%20-%20테이블의%20개념과%20정규형/images/업계%20테이블의%20엔터티.jpg)
...
![고객기업 테이블의 엔터티](https://github.com/0jun0815/YJStudy/blob/master/데이터베이스/테이블%20설계의%20기초%20-%20테이블의%20개념과%20정규형/images/고객기업%20테이블의%20엔터티.jpg)


'FK' 키워드는 'Foreign Key(외래키)'의 약칭이다. '외래키'란 '다른 테이블에서도 같은 의미로 사용되는 열'을 의미한다. 고객기업 테이블의 '업계코드'는 '업계 테이블의 기본키'에도 있다. 즉, '업계 테이블에 존재하지 않는 업계는 고객기업 테이블에도 존재할 수 없다'라는 의미이다. 이것은 테이블에 걸친 제약이어서 '외래키 제약'이라고 불린다. 이 외래키야말로 이 ER 다이어그램의 핵심이 되는 요소이다.


&nbsp;
### 릴레이션십이란
엔터티 간의 관련성을 표현하는 것이 앞에서 설명한 '외래키의 존재'이다. 외래키가 존재하는 테이블은 해당 열이 다른 테이블의 기본키 열(또는 기본키의 일부 열)을 참조하는 것을 의미한다. 이 관련성을 '릴레이션십'이라고 한다. 업계 테이블과 고객기업 테이블과의 사이에서는 '일대다'(1대 N이라고도 한다)의 관계가 존재한다. 업계 테이블의 1행, 즉 1개의 업계에 대해 고객기업 테이블의 복수 기업이 대응하고 있다는 뜻이다. 즉 '한 업계에는 복수의 기업이 소속된다'라는 포함관계를 표현하는 것이므로 '귀속관계'라고 표현할 수도 있다.


IE 표기법에서는 아래 그림에 나타낸 규칙과 기호로 테이블 간 레코드 수의 대응관계를 표시한다.


![IE 표기법에 기반을 둔 레코드 수 표기법](https://github.com/0jun0815/YJStudy/blob/master/데이터베이스/테이블%20설계의%20기초%20-%20테이블의%20개념과%20정규형/images/IE%20표기법에%20기반을%20둔%20레코드%20수%20표기법.jpg)


&nbsp;
### 실제로 ER 다이어그램을 작성해 보자
업계 테이블은 고객기업 테이블에서 복수의 레코드와 연결될 가능성이 있고 고객기업 테이블에는 아직 기업이 등록되어 있지 않은 업계(대응 하는 기업의 수가 '0'인 경우)가 있다는 가정하에 '0(O)'과 '복수(새의 다리)'의 기호를 조합하여 다이어그램을 작성한다. '고객기업 테이블'에서 보면 '업계'는 항상 한 개의 레코드로 대응한다. 따라서 '1(-)'의 기호를 사용한다.


![ER 다이어그램 완성판](https://github.com/0jun0815/YJStudy/blob/master/데이터베이스/테이블%20설계의%20기초%20-%20테이블의%20개념과%20정규형/images/ER%20다이어그램%20완성판.jpg)


&nbsp;
### 2가지 중요한 핵심
테이블 설계에서 가장 중요한 것은 '테이블이 집합인 것'과 '테이블이 함수인 것'을 이해하는 것이다. 정규화나 ER 다이어그램은 테이블이 집합이며 함수라는 것을 재확인하기 위한 보조적인 기법, 사고의 보조선에 지나지 않는다.
